...
{
$query=mysql_query("select * from sd_registration where `email`='".$_REQUEST['username_login']."' and `password`='".$_REQUEST['password_login']."'");
...
<a style=color:blue Href='index3.html'>click here to login</a>
</td>
</tr></table>
<?php
}
...
{	
	...
	echo "<script>parent.location.href='../category_main.php';</script>";
}
}
}
else{
...
//echo $code;
$query=mysql_query("select * from `sd_registration` where `email`='".$_POST['email']."'");
if(mysql_affected_rows()<1){
$query="INSERT INTO `sd_registration` (`username`, `email`, `password`, `code`) VALUES 
